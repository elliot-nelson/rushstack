...
export class SelectorScopeError extends Error {
  ...
  public constructor(message: string, scope: string, value: string) {
    super(message);
    this.scope = scope;
    this.value = value;
  }

  public isSelectorScopeError(error: Error): error is SelectorScopeError {
    return !!(error && (error as SelectorScopeError).scope);
  }
}
